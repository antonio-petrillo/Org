...
* Lezione 3
Introduciamo dei modi per limitare il numero di insiemi resto $R_n$ da generare per controllare se $X$ è codice con Sardinas & Patterson.
** Insieme di suffissi finito :lemma:
...
$$\blacksquare$$
** Limiti per gli i resti destri $R_n$ (vale dualmente per i resti sinistri $L_n$)
...
** Teorema di Levenshtein :theorem:
